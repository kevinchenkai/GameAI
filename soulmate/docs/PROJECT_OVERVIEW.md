...
每日心动值不是固定从 0 开始。前端每天按日期种子生成一个基础心动值（公式 `52 + stableDaySeed(today) % 29`，即 52~80），再通过点击照片、长按照片和聊天继续增长。
...
>
> **仓库结构**：`soulmate/` 本身不是独立 git 仓库，而是 `GameAI` 仓库（git root = `/Users/kk/Work/GameAI`）下的子目录，与 `Tavern/` 等子项目并列。`.gitignore`（含 `.env`、`.checks/` 等忽略规则）由 `GameAI` 父仓库统一管理，`soulmate/` 目录下没有独立的 `.gitignore`。
...
公开接口（`/api/chat`、`/api/uid/*`、`/api/health` 等）返回宽松 CORS（`Access-Control-Allow-Origin: *`）；Admin 记忆类接口（`/api/memory`、`/api/context/preview`）**不返回跨域 CORS 头**，按同源使用，除 token 校验外再加一层防跨域调用。
...
| `delta` | `{ delta, fullText }` | 逐段文本（DeepSeek 回复先在服务端缓冲并通过截断检测后，才分片回放为 `delta`，不会把半截/被判定截断的回复先泄漏给前端） |
| `done` | `{ ok, reply, mood, heartDelta, memoryUsed, source, time }` | 完成 |

> 说明：
> - `mood` 是把客户端传入的 `mood` 原样回传（非法值回退为 `cute`），后端不做情绪判定，表情切换由前端 `detectMood()` / `resolveMood()` 负责。
> - 前端发送消息时会先按互动类型即时增加一次心动值；收到回复后再读取 `done.heartDelta` 追加一次。后端当前恒定返回 `2`（deepseek）/`1`（fallback），前端本地 fallback 使用 `2`。
...
`deepseek.js` 先把整段回复缓冲在服务端，**流结束后**再检测：
...
任一命中则抛错，由 `server.js` 兜底为服务端规则回复（`source: "fallback"`）。
检测通过后，缓冲的完整回复才会分片回放为 SSE `delta`，因此前端不会先看到半截回复再被替换。
