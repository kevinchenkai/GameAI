...
CASES = [
    ...
    ("T-001", "江南古寺 · 阴雨天旅行纪录片", "T-001-jiangnan-temple.txt", "T2VA",
     ...
     "原创 T2VA 作品：从中文 brief 规范化为官方三字段 prompt，无任何输入图片。"
    ...
    ("T-003", "酒红玫瑰墙 · 自拍姿态 V4B", "T-003-rose-selfie.txt", "I2VA",
     "576×1024", 243, "10.125s", "部分成功", "warn",
     "竖屏 I2VA 原创单镜头，以指定人脸首帧锚定身份。技术验像通过：243 帧、无黑帧/NaN、无镜头切换。"
     "V4B 将原本的对称托脸改为高位梳发 + 低位胸前发梢手，并在 wink 前将低手放回腿部；"
     "选定样本出现单眼 wink 且无下巴托手。配对实验确认手势结构改善，但 wink 时长与时机仍有 seed 波动，"
     "因此作为可观看原型如实收录，未标为验收交付。"),
]
