import { assertDeepSeekReady, config } from './config.js';
...
export async function streamDeepSeek(messages, onDelta) {
  ...
  try {
    ...
    if (!response.body || typeof response.body.getReader !== 'function') throw new Error('DeepSeek stream unavailable');
...
async function readDeepSeekStream(response, onDelta) {
  ...
  if (!fullText.trim()) throw new Error('DeepSeek empty stream');
