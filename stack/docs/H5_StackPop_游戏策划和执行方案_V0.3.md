...
### 13.2.2a 目标带必须低于硬上限 ★ V0.3.3 新增

**生成器的目标是 30%，不是 35%。** 35% 是 build fail 的红线，
若关卡贴着红线生成，**任何测量噪声都会让构建随机失败**。

```text
生成器目标带：L6~L20 落在 [目标值 ±3pp]，最高关目标 30%
硬上限：35%（留 5pp 安全边际）
```

**这不是保守，是必需。** 实测证据（V0.3.3 复核）：

M3 复修后 L13/L16/L17/L19 的真实失败率都在 **34% 左右**，紧贴上限。
在 `trialsPerStrategy = 200` 下重复测量 L19 二十次：

```text
38 38 38 33 32 40 36 35 35 34 37 41 30 35 34 34 34 34 31 31
范围 30.5% ~ 41.0%，其中 7/20 次超过 35%
```

**同一份关卡数据，换个 trial seed 就有 35% 概率 build fail。** 这是不可接受的。

### 13.2.2b 模拟样本数必须提高 ★ V0.3.3

```ts
// config/tuning.ts
trialsPerStrategy: 1000,   // V0.3.3：原为 200
```

| N | 34% 处的 95% 置信区间 |
|---:|---|
| 200 | ±6.6pp → [27.4%, 40.6%] ❌ |
| **1000** | **±2.9pp → [31.1%, 36.9%]** ✅ |
| 4000 | ±1.5pp | 过慢，不必要 |

**N=1000 且目标 30% 时**：95%CI = [27.2%, 32.8%]，上界远低于 35%，构建稳定。

代价是 `validate-levels` 耗时约 ×5（20 关 ~1s → ~5s），完全可接受。

> 📌 **通用教训**：任何「阈值判定 + 随机测量」的组合，
> 都必须让**目标值与阈值之间留出大于测量噪声的间距**，否则 CI 会随机变红。

> **35% 的含义**：平均打 1.5 次过关。玩家失败一次立刻再来一次就过，
> 这是休闲游戏可接受的上限。超过 50% 意味着「过半数尝试以失败告终」，
> 对休闲用户是明确的劝退信号。
...
✓ 曲线单调不降（相邻差 <= 12pp 触发 WARNING）  ★ V0.3.2
...
| `simulate` | 三种 Bot 失败率；**L1~5 <= 5%、L6~20 <= 35%、曲线单调不降** |
