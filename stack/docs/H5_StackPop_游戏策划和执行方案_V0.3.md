...
maxColumnDepth = 12    // 硬上限，validate 时拦截（V0.3.1：原为 10）
```
...
totalTileCount % 3 === 0
count(每种 type) % 3 === 0
maxColumnDepth <= 12                                  ★ V0.3.1：原为 10
columnCount <= 6                                      ★ 教学关 1~3 可用 4/5 列
totalTileCount <= columnCount * maxDepth * 0.88       ★ V0.3.1 新增：容量松弛
```

违反任一条 → `validate-levels` 报错，不允许进入构建。

> **最后一条是 V0.3.1 新增的关键约束。** 它比「不超容量」更严：Tile 数逼近容量会导致
> 所有列等高填满，同时失去视觉参差、策略空间与生成器自由度。详见 §10.0。
...
# 10. 前 20 关难度曲线 ★ V0.3.1 修订（容量冲突已修）

## 10.0 修订说明：原表 L11~20 物理上不可能

**原表存在硬冲突，L11~20 共 10 关无法实现**（Codex 在 M3 开工前发现）：

```text
固定 6 列 × 最大深度 10 = 60 张（容量上限）
但原表 L11=66、L13=72、L15=78、L18=84、L20=90 张
```

这是我的错误：先定了 Tile 数（沿用 V0.2 以「总数」为主轴的思路），
后来把列数从 8 锁到 6、深度锁到 10，**没有回头重算容量**。
...
| 选项 | 代价 | 结论 |
|---|---|---|
| 放弃「固定 6 列」 | 375px 屏 8 列 tileSize=39px < 48 下限，且需两套关卡数据 | ❌ 不可行 |
| 放弃「Tile 数爬升」（全封 60） | L9~L20 共 12 关局长全是 81s，时长曲线被压平 | ⚠️ 可行但难受 |
| **放宽「深度 ≤ 10」到 12** | 需重验布局 | ✅ **采用** |

**布局已实测**（`calculateGameLayout`，四个目标 viewport）：

| 深度 | 360×800 | 375×812 | 390×844 | 430×932 |
|---:|---|---|---|---|
| 10 | ✅ tile 49.7 余量 155 | ✅ 52.2 / 140 | ✅ 54.7 / 143 | ✅ 61.3 / 158 |
| 11 | ✅ 49.7 / 113 | ✅ 52.2 / 96 | ✅ 54.7 / 97 | ✅ 61.3 / 106 |
| **12** | ✅ **49.7 / 71** | ✅ **52.2 / 51** | ✅ **54.7 / 50** | ✅ **61.3 / 53** |
| 13 | ⚠️ tile 降到 48.8 | ⚠️ 49.4 | ⚠️ 51.6 | ⚠️ 58.1 |

**深度 12 在全部四个尺寸下都保持满 tileSize，余量 ≥ 50px。深度 13 开始 tile 被压缩，故上限定为 12。**

### 关键约束：容量不得用满 ★

```text
maxColumnDepth = 12  →  容量 72 张
但 tileCount 上限 = 63 张（约 88%）
```

**理由：Tile 数若逼近容量，所有列必须等高填满，会同时失去三样东西：**

1. **视觉特征** —— 原型图中各列高度参差、中间留空，这是本品类的视觉语言
2. **策略空间** —— 等高棋盘让「哪列先挖」的决策价值下降
3. **生成器自由度** —— `targetDepthSpread`（§9.2 第 2 号旋钮）需要高度差才能调

> 📌 **`validate-levels` 必须新增一条硬校验：`tileCount <= columnCount * maxDepth * 0.88`**，
> 违反即报错。这条比「≤ 容量」更严，是为了把上面三样东西保住。

---

## 10.1 修订后的难度曲线

| 关卡 | 图案种类 | Tile 数 | 列数 | 最大列深 | 容量 | 目标时长 | 设计目标 |
|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 3 | 18 | 4 | 6 | 24 | 25s | 教「只点顶部」 |
| 2 | 3 | 24 | 5 | 7 | 35 | 32s | 教「三枚同类自动消除」 |
| 3 | 4 | 30 | 5 | 8 | 40 | 40s | 教「槽位可暂存不同类」 |
| 4 | 4 | 36 | 6 | 8 | 48 | 49s | 第一次需要观察全局 |
| 5 | 4 | 42 | 6 | 9 | 54 | 57s | 巩固 |
| 6 | **5** | 42 | 6 | 9 | 54 | 57s | ⚠️ 首个真实难度台阶 |
| 7 | 5 | 45 | 6 | 9 | 54 | 61s | 槽位开始有压力 |
| 8 | 5 | 48 | 6 | 10 | 60 | 65s | 首个明显陷阱 |
| 9 | 5 | 48 | 6 | 10 | 60 | 65s | 需要撤回意识 |
| 10 | **6** | 51 | 6 | 10 | 60 | 69s | ⚠️ 第二个台阶 |
| 11 | 6 | 51 | 6 | 10 | 60 | 69s | 中等 |
| 12 | 6 | 54 | 6 | 11 | 66 | 73s | 中等 |
| 13 | 6 | 54 | 6 | 11 | 66 | 73s | 长局 |
| 14 | 6 | 57 | 6 | 11 | 66 | 77s | 顶部干扰 |
| 15 | 6 | 57 | 6 | 11 | 66 | 77s | 中高 |
| 16 | 6 | 60 | 6 | 12 | 72 | 81s | 中高 |
| 17 | **7** | 60 | 6 | 12 | 72 | 81s | ⚠️ 第三个台阶 |
| 18 | 7 | 60 | 6 | 12 | 72 | 81s | 陷阱 |
| 19 | 7 | 63 | 6 | 12 | 72 | 85s | 需要规划 |
| 20 | 7 | 63 | 6 | 12 | 72 | 85s | 阶段挑战 |

**全部 20 关满足 `tileCount <= 容量 × 0.88`，且 `tileCount % 3 == 0`。**

**曲线说明：**

- **1~2 关**：3 种图案 → `3×2 = 6 < 7 格`，**数学上不可能失败**
- **3~5 关**：4 种图案 → 理论可死（8>7），实际需极端凑巧。软着陆区
- **第 6、10、17 关**：三个明确台阶，各引入 1 种新图案。**台阶靠图案种类，不靠 Tile 数**
- **Tile 数**：18 → 63 平滑爬升，对应时长 **25s → 85s**，只控时长不控难度
- **列深**：5 → 12 渐进加深，配合 `targetDepthSpread` 制造陷阱
- **列数**：L1~L3 用 4/5 列（教学期减少信息量），L4 起固定 6 列

> 每次台阶（6/10/17）的前一关应刻意做得轻松，形成「放松—紧张」节奏。

## 10.2 局长缩短的取舍

原表目标 L20 = 130s，修订后 85s。**这是接受的代价**，理由：

- 原目标 130s 本就基于「90 张」这个**物理上不可能**的数字
- §1 定义单局 1~3 分钟，85s 仍在区间内且偏舒适
- 本品类的疲劳点是**槽位压力**而非局长；靠拉长局长堆难度会让后期关卡变得冗长
- 真正的难度来源是图案种类（§9.2 第 1 号旋钮），这条完全没受影响
...
## 11.2 难度定向控制 ★ V0.3.2：depthSpread 是唯一有效旋钮

**实测结论（本文档编写时验证）**：固定规格「7 种 / 60 张 / 6 列 / 深 12」下，
**只换 seed 重排完全无法控制难度**——12 个种子的 greedy 失败率全部落在 87%~100%。

真正起作用的是**同类三张的深度差 `depthSpread`**：

| depthSpread | greedy 失败率区间 | 中位 |
|---:|---|---:|
| 0~1 | 0% ~ 11% | 6.7% |
| **2** | **13% ~ 39%** | **28.7%** |
| 3 | 29% ~ 85% | 48.7% |

**因此生成器必须在 `depthSpread` 上搜索，而不是只在 seed 上搜索。**
目标区间 ≤35% 对应 `depthSpread ≈ 1~2`，配合 seed 微调。
...
## 13.2 第 2 层：Simulator（人类可通）★ V0.3.2 重定阈值：休闲优先

```bash
npm run simulate
```

**Codex 的关键提醒（已采纳）：`simDeadlockRate` 是「当前 Bot 策略在该关的失败率」，不是真人失败率。**
若直接 `>45% → build fail`，Generator 会**专门适应 Bot 而不是适应真人**。

### 13.2.1 产品定位：这是一款休闲游戏 ★

**V0.3.2 明确定位：以轻松为主，人类玩的时候不要太难。**

这条定位直接改写难度阈值。原 V0.3 的「≤70% 才 fail」是**上限思维**——只防最坏情况，
不管曲线形状。休闲定位下必须改成**目标思维**：每关都应落在一个窄区间内。

**为什么按「不会撤回的玩家」设计：**

Undo 无限免费（§6.2），理论上玩家撞到 tray 满可以撤回换路，所以 bot 的 failRate 高估了挫败感。
**但休闲定位下仍应按「不撤回」来设计**：

1. 休闲玩家未必想到用撤回，也未必愿意用（有心理成本）
2. §15 星级把「0 次撤回」定为 ★★★，等于在**暗示玩家别撤回**
3. **撤回是安全网，不该是通关的必要条件**
...
| 关卡 | Greedy 目标 | 硬上限 | 说明 |
|---:|---:|---:|---|
| 1~3 | **0%** | 2% | 纯教学，3 种图案数学保证不可能失败 |
| 4~5 | ~3% | **5%** | 软着陆 |
| 6~9 | 8% → 14% | **35%** | 首个台阶后缓升 |
| 10~13 | 16% → 21% | **35%** | 中段 |
| 14~16 | 22% → 25% | **35%** | 中高 |
| 17~20 | 26% → 30% | **35%** | 目标最高 30%，硬上限 35%（留 5pp 边际）|

**三条硬性判定：**

```text
❌ BUILD FAIL：L1~L5  greedy > 5%
❌ BUILD FAIL：L6~L20 greedy > 35%     ← 硬上限
⚠️ WARNING  ：相邻两关 greedy 差值 > 12pp（曲线抖动）
```

### 13.2.2a 校验规则必须比测量噪声宽 ★ V0.3.4 重写

**V0.3.3 的「目标值 ±3pp」与「逐关严格单调」都是错的规格**，我写窄了。
两者都比测量噪声还紧，导致**关卡数据没问题，构建却随机失败**。

实证（V0.3.4 复核，用 Codex 未测过的 seed 跑 `validate-levels`）：

```text
--trial-seed=20260823 → 0/20 通过
  L11: 0.211 outside target 0.18 ±0.03
  L12: curve decreased from 0.211 to 0.205      ← 差 0.6pp，纯噪声
  L19: 0.335 outside target 0.29 ±0.03
--trial-seed=555      → 0/20 通过
  L13: curve decreased from 0.225 to 0.224      ← 差 0.1pp，纯噪声
```

**根因：N=1000 时，单点噪声 ±2.8pp，两点之差的噪声 ±4.0pp。**
用 ±3pp 的容差去卡一个噪声 ±2.8pp 的量，等于抛硬币。

### 13.2.2b 正确的三条校验 ★ V0.3.4

**原则：校验「关卡是否够休闲」，不是「是否精确命中某个数字」。**
...
B. 带容差的单调（容差必须 > 两点差的噪声 ±4pp）
   对 L6~L20：rate[i+1] >= rate[i] - 5pp
   即允许 5pp 以内的回落（那是噪声，不是设计问题）

C. 分段递增（替代逐关比较，噪声被 sqrt(段长) 除掉）
   avg(L6~L10) < avg(L11~L15) < avg(L16~L20)
```

**删除「目标值 ±3pp」这条硬校验。** 目标值仅作为生成器的**搜索方向**保留，
不作为构建门禁——生成器该瞄准 30%，但测出 32% 不该让构建失败。

**稳健性已验证**：用 8 个独立 seed 各跑一遍全套校验，**8/8 通过**。

> 📌 **通用教训（第二次踩）**：随机测量上的门禁，
> **容差必须大于噪声，且噪声要按「差值」而非「单点」算**（差值噪声是单点的 √2 倍）。
> 相邻目标只差 1~3pp 时，逐点比较必然不稳定，应改用分段聚合。

### 13.2.3 为什么不再用「≤70%」

按 V0.3 的旧门槛，M3 产出的 20 关全部合格，但实测体感是：

```text
L12 51%  L15 49%  L17 66%  L18 62%  L20 53%   ← 5 关过半失败
普通玩家通关全 20 关平均需要打 30 局
其中 L12/L17/L18/L20 四关就吃掉 10 局
```

按 V0.3.2 目标曲线，同样 20 关**平均 24.5 局**，且最难关 34%。

### 13.2.4 曲线必须单调不降

原 M3 曲线存在 4 处 10pp 以上回落，最大 L18(62%) → L19(28%) 落差 **34pp**。
玩家体感是「忽难忽易」，而非稳步进阶。

```text
要求：target[i] <= target[i+1]（允许持平，不允许明显回落）
容差：相邻差值 > 12pp 触发 WARNING
```

> 未来拿到真人数据后，做 `humanFailRate vs greedyFailRate` 相关性分析，再校准本表。
...
> 🔴 **这是一条可校验的硬约束，`validate-levels` 必须检查：**
> ```text
> L3 初始顶部：任意 type 的计数必须 <= 2
> ```
> M3 首版 L3 的初始顶部是 `[watering, grass, paw, paw, paw]`——**三个 paw，
> 玩家一上来就能直接三消**，学不到「暂存」这件事，第一印象仍停留在「找三个一样的」，
> 而这正是 V0.1 那个错误规则的心智模型。L3 是 20 关里唯一负责教这件事的关卡，必须改对。
...
## 19.3 深度可行性校验（已实算，V0.3.1 扩到 12）

**M0 实测数据**（`calculateGameLayout`，OVERLAP_RATIO = 0.85）：
...
| 11 | 49.7 / 113 | 52.2 / 96 | 54.7 / 97 | 61.3 / 106 |
| **12** | **49.7 / 71** | **52.2 / 51** | **54.7 / 50** | **61.3 / 53** |
| 13 | ⚠️ tile 压到 48.8 | ⚠️ 49.4 | ⚠️ 51.6 | ⚠️ 58.1 |

**深度 12 在四个尺寸下均保持满 tileSize，余量 ≥ 50px；深度 13 起 tile 被压缩。
因此最大列深锁定 12。**
...
```text
手机：用色块渲染 6 列 × 深度 12 的棋盘，在四个 viewport 截图确认不溢出
...
✓ 列深 <= 12 / columnCount <= 6
✓ tileCount <= columnCount * maxDepth * 0.88   ★ V0.3.1 新增
✓ Solver 可以完成
✓ solution 逐步重放：每步 expectedTileType 与实际顶部一致  ★ P0-2
✓ solution 重放到最后 columns 与 tray 均为空
✓ Level 1~5 的 Greedy 失败率 <= 5%
✓ Level 6~20 的 Greedy 失败率 <= 35%          ★ V0.3.2 休闲定位
✓ 曲线带 5pp 容差的单调不降                    ★ V0.3.4
✓ 分段递增 avg(L6~10) < avg(L11~15) < avg(L16~20)  ★ V0.3.4
✓ L3 初始顶部任意 type 计数 <= 2               ★ V0.3.2
```
...
- [ ] **Level 1~5 Greedy 失败率 <= 5%；Level 6~20 <= 35%**（V0.3.2 休闲定位）
- [ ] **曲线带 5pp 容差单调不降；分段递增成立**
- [ ] **`validate-levels` 在任意 trial seed 下都通过（不只是选定的几个）**
- [ ] **L3 初始顶部任意 type 计数 <= 2**（教学关必须逼出「暂存」）
...
18. ★ Hint 只能基于当前状态做四级启发式（文档 §8.2），
    ...
    那一步可能指向空列，甚至加速进入死局。
18.5 ★★ 本作是【休闲游戏】，人类玩的时候不要太难。难度阈值（V0.3.2）：
    - L1~L5  greedy <= 5%
    - L6~L20 greedy <= 35%（不是旧版的 70%）
    - 曲线必须单调不降，相邻两关差值 > 12pp 触发 WARNING
    - 难度靠 depthSpread 调；只换 seed 无效（实测 12 个种子全在 87%~100%）
    - L3 初始顶部任意 type 计数 <= 2，否则玩家学不到「暂存」
...
24. MVP 棋盘固定 6 列（教学关 1~3 可用 4/5 列），最大列深 12，tileSize 范围 [48,92]。
    ★ 关卡 Tile 数必须 <= 列数 × 最大深度 × 0.88（容量松弛，见 §10.0）。
...
M0：项目骨架（复用 garden/web）+ Boot/Preload/Home/GameScene
    ...
    ★ 额外必做，布局可行性验证，不能推迟到 M4：
      - 手机：色块渲染 6 列 × 深度 12，在 4 个目标 viewport 截图确认不溢出
...
| `simulate` | 三种 Bot 失败率；**L1~5 <= 5%、L6~20 <= 35%** |
| `validate-levels --trial-seed=<随机值>` | **换任意种子都必须通过**（防噪声门禁） |
