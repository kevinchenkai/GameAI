 ...
 * 特殊棋子诞生的"脉冲"。
 *
 * ★ 是**放大回弹**不是迸粒子 —— 这一刻的语义是"得到了一个好东西"，
 *   粒子那种往外炸的语义正好相反。
 *
 * ★ 幅度克制（1.25）：它紧跟在一次消除之后，
 *   弹得太夸张会盖过刚刚那次消除本身的反馈。
 */
export const SPECIAL_SPAWN = {
  ...
  peakScale: 1.25,
  /**
   * ★★ 自带时长，**不用 timeline 给的 durationMs**。
   *
   *   `specialSpawn` 在 timeline.ts 里是**零时长的瞬时标记**
   *   （它不该推迟整段动画的节奏）。若脉冲照抄 durationMs，
   *   补间时长就是 0 —— 一帧闪过，等于没做。
   *   所以这里独立给一个短时长，它与时间轴并行、不占用它。
   */
  riseMs: 130,
  /** 去程占脉冲总时长的比例（剩下的是回弹） */
  riseRatio: 0.45,
