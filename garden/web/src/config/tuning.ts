...
export const LAYOUT = {
  ...
  minPieceSizePt: 38,
  /**
   * ★★ 棋子边长**上限**（pt）—— 电脑浏览器适配的关键一条。
   *
   *   ⚠️ 原本只有下限没有上限，后果是在宽屏上棋盘无限放大：
   *   实测 1280×720 笔记本上棋子 98pt、1920×1080 上 150pt
   *   （手机上只有 49pt），棋盘吃掉 688~1048px 宽，
   *   而**剩余高度被压到 0 —— HUD、旺财、设置按钮全部塌掉**。
   *   用户看到的"header 和 settings 很乱"就是这么来的。
   *
   *   64pt 约为大手机（414pt 宽）上的观感上限：
   *   再大不会更好认，只会把其它区域挤没。
   */
  maxPieceSizePt: 64,
  /**
   * ★ 内容区最大宽度（pt）。超出部分留白，整体居中。
   *
   *   竖屏手机游戏在宽屏上**不该拉伸铺满** —— 那会让 HUD 两端
   *   甩得极远、旺财浮在正中间一大片空白里。
   *   限宽后桌面上呈现为"居中的一条手机版式"，是这类游戏的通行做法。
   */
  maxContentWidthPt: 480,
  /** 依次尝试的棋盘边长 */
