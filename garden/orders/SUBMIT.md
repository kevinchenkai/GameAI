...
## 指令 5 —— 叠加层风格重做（3 张）🎨 纯观感

> 用户实测："横向、纵向的箭头，炸弹，好难看，这个要重点优化。"
> ⚠️ 上一版**可读性已达标**，这次只改风格，不能让可读性倒退。

⬛

```
请执行 Garden Match 的第 5 批出图工单（叠加层风格重做）。

工单文件（请完整阅读后再动手）：
  garden/orders/codex-redo-batch5-overlays-style.md

出图前必读：
  garden/orders/LEGIBILITY-SPEC.md —— 尤其 §5.2「遮挡层与标记层规则相反」

要点：
1. 本批只重出 3 张：overlay-rocket-h / overlay-rocket-v / overlay-bomb
   路径与文件名**不变**（同名覆盖），代码零改动
2. 这是**纯观感重做**。上一版功能与可读性已验收通过，
   这次要解决的是"标记与水果不是一套画风"

3. ★ 关键诊断（工单 §2）：问题**不是"太扁平"**，是**对比过硬**。
   实测明暗跨度：标记 210~233，水果只有 38~51 —— 相差 4~6 倍。
   标记从纯深棕直接跳到亮金，中间没有过渡；水果是窄区间里的柔和渐变。
   **不要按"加点立体感"去改**，要按"把对比收进水果那个区间"去改。

4. 🔴 硬判据不许倒退（工单 §4）：
   · 深色描边 + 亮色芯的双色结构**必须保留**（柔化 ≠ 去掉）
   · 标记与其下棋子灰度差 **≥ 20，六色全部满足**
   · 判据是"标记看不看得见"，**不是**"棋子还认不认得出"
   · 重点自查**香蕉（yellow）**——最浅，历史上就是在这里失败的（Δ 曾只有 5.7）

5. 火箭的**方向必须一眼看出**（横向指左右、竖向指上下）——
   这关系到玩家能否预测清除范围，M8 要专门观察理解率

6. 交付**未压缩原图**，压缩由代码侧统一做

若"柔和风格"与"灰度差 ≥ 20"发生冲突，**停下来报告，不要硬凑**。
优先级：可读性 > 风格统一。宁可保留现在这版，
也不要交一个"好看但看不见"的标记。
```
