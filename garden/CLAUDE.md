...
**首屏加载**：✅ **已解决**（2026-08-08）。**4276 KB → 206 KB，降 95%**。
详见 [TODO-性能优化.md](./docs/TODO-性能优化.md)。三步：院门移出 `BootScene`（−3064KB）、
**删掉 6 张从没被画过的贴图**（−660KB，占当时首屏 54%）、pngquant 压缩（−63%）。
...
| 事项 | 优先级 | 说明 |
|---|---|---|
| ~~L6~L8 死局率 8%~~ | ✅ 已解决 | 从源头修（refill 避免制造死局），**8% → 0.5%**，保留 6 色与难度曲线 |
| ~~特殊棋子叠加层美术~~ | ✅ 已解决 | 第 6 批重做通过验收；⚠️ 再改这三张图**必须改名到 `-v3`**（30 天图片缓存） |
| 旺财 thinking/hint 专属动作 | 🟡 用户说"放到后面" | 逻辑正确，视觉上只有 Idle 与跳跃 |
| `candidates/` 清理（110MB）| 🟡 M8 之后 | 已 gitignore、服务器 0 个、零引用。留着以便测试后直接切换候选 |
